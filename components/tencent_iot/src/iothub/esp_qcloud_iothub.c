...
esp_err_t esp_qcloud_iothub_report_all_property(void)
{
    ...
    ESP_QCLOUD_ERROR_CHECK(err != ESP_OK, err, "<%s> esp_qcloud_handle_get_param", esp_err_to_name(err));
...
static void esp_qcloud_iothub_log_callback(const char *topic, void *payload, size_t payload_len, void *priv_data)
{
    ...
    if (!strcasecmp(cJSON_GetObjectItem(request_data, "type")->valuestring, "get_log_level") && cJSON_GetObjectItem(request_data, "log_level"))
    {
...
esp_err_t esp_qcloud_iothub_register_log()
{
    esp_err_t err = ESP_OK;
    char *publish_topic = NULL;
    char *publish_data = NULL;
...
esp_err_t esp_qcloud_iothub_report_device_info(void)
{
    ...
    cJSON_AddStringToObject(device_label, "manufacturer", "新工科产业学院");
