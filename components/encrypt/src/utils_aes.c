 ...
 */
                      ...
                      uint8_t *pKey, uint16_t keybits, uint8_t *iv)
    {
        ...
        if (UTILS_AES_ENCRYPT == mode)
        {
            ...
            if (ret != 0)
            {
                printf("Set encry key err,ret:%d\r\n", ret);
                ret = -1;
                goto exit;
            }
            ...
            {
                printf("Output buffer should not less than datalen+UTILS_AES_BLOCK_LEN for padding\r\n");
                ret = -1;
                goto exit;
            }
        ...
        else
        {
            ...
            if (ret != 0)
            {
                printf("Set dencry key err,ret:%d\r\n", ret);
                ret = -1;
                goto exit;
            }
        }
    ...
    int aes_sample(int verbose)
    {
        ...
        if (0 == utils_aes_cbc((uint8_t *)dataIn, sLen, (uint8_t *)dataIn, sLen + UTILS_AES_BLOCK_LEN,
