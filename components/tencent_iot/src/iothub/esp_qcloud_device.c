...
static char g_mac_str[31] = {0};
static char g_device_secret[DEVICE_SECRET_SIZE + 1] = {0};
#endif
...
#ifdef CONFIG_AUTH_MODE_DYNREG
    strncpy(g_device_secret, device_secret, DEVICE_SECRET_SIZE);
    g_device_secret[DEVICE_SECRET_SIZE] = '\0';
    g_device_profile->device_secret = g_device_secret;
#endif
...
#ifdef CONFIG_AUTH_MODE_DYNREG
    g_device_profile->auth_mode = QCLOUD_AUTH_MODE_KEY;
    g_device_profile->product_secret = CONFIG_QCLOUD_PRODUCT_SECRET;
    /**
     * @brief Get MAC address
     * 
     * @note Set device name as MAC address.
     */
    uint8_t mac[6] = {0};
    esp_read_mac(mac, ESP_MAC_WIFI_STA);
    sprintf(g_mac_str, "%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    g_device_profile->device_name = (char *)g_mac_str;
...
esp_qcloud_profile_t *esp_qcloud_get_device_profile()
{
